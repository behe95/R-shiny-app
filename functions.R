...
get_conf<-function(
  request,
  configuration_file="./misc_files/configuration.R"
){
...
prepare_individual_genome<-function(
  ...
  if(uniqueID%in%list.files("./data/")){  #Also check for pre-existing uniqueIDs and stop if so (never happened though)
  ...
  if(!is_vcf_file){
    ...
    if(!file.exists("./vcfs/"))dir.create("./vcfs/")
    homeFolder<-paste("./vcfs/",homeFolderShort,"/",sep="")
    dir.create(homeFolder)
    setwd(homeFolder)
    write.table("Job is not ready yet",file="job_status.txt",col.names=F,row.names=F,quote=F)
  ...
  all_md5sums_path<-"./misc_files/md5sums.txt"
  if(!file.exists(all_md5sums_path)){write("md5sums",file="./misc_files/md5sums.txt")}
  ...
  queue_length <- length(list.files("./imputations/"))
  if(is_vcf_file){
  ...
  if(get_conf("running_as_docker")){
    ...
    if(!file.exists("./misc_files/supercronic.txt"))stop(safeError("Code was found to be running as docker container, but with no misc_files/supercronic.txt file ready for launch. The job will likely never execute."))
    ...
    supercronic_out<-try(system("supercronic ./misc_files/supercronic.txt &"))
...
prepare_imputemany_genome<-function(
  ...
  acceptedMails_path<-"./misc_files/accepted_emails.txt"
  ...
  if(should_be_imputed){
    ...
    for(uniqueID in names(sampleNames)){
      ...
      homeFolder<-paste("./imputations/imputation_folder_",uniqueID,"/",sep="")
      dir.create(homeFolder)
    ...
    imputemany_registry_path <- "./misc_files/imputemany_registry.txt"
...
check_for_cron_ready_jobs<-function(
  ...
  if(job_type=="single"){
    ...
    if(serverRole== "Node"){
      ...
      cmd1 <- paste("ssh ubuntu@",hubAddress," ls -l --time-style='+\\%Y-\\%m-\\%d-\\%H:\\%M:\\%S' ./imputations/  | tail -n +2",sep="")
      ...
      cmd0 <- paste("ssh ubuntu@",hubAddress," cat ./misc_files/fast_queue_emails.txt
                ",sep="")
      ...
      for(remoteFolderToCheck in remoteFoldersToCheck){
        cmd2 <- paste("ssh ubuntu@",hubAddress," cat ./imputations/",remoteFolderToCheck,"/job_status.txt",sep="")
        ...
        if(job_status=="Job is ready"){
          ...
          cmd3 <- paste("ssh ubuntu@",hubAddress," 'echo Job is remote-running > ./imputations/",remoteFolderToCheck,"/job_status.txt'",sep="")
          system(cmd3)
          
          #then copy all the files to here
          cmd4 <- paste("scp -r ubuntu@",hubAddress,":./imputations/",remoteFolderToCheck," ./imputations/",remoteFolderToCheck,sep="")
          system(cmd4)
    ...
    if(serverRole== "Node"){
      ...
      if(length(remoteFoldersToRun)==length_requested){
        ...
        for(remoteFolderToRun in remoteFoldersToRun){
          cmd3 <- paste("ssh ubuntu@",hubAddress," 'echo Job is remote-running > ./imputations/",remoteFolderToRun,"/job_status.txt'",sep="")
          system(cmd3)
        }
        
        #then copy all the files to here
        for(remoteFolderToRun in remoteFoldersToRun){
          cmd4 <- paste("scp -r ubuntu@",hubAddress,":./imputations/",remoteFolderToRun," ./imputations/",remoteFolderToRun,sep="")
          system(cmd4)
          
          #And write locally that job is ready
          job_status_file<-paste("./imputations/",remoteFolderToRun,"/job_status.txt",sep="")
          unlink(job_status_file)
          write.table("Job is ready",file=job_status_file,col.names=F,row.names=F,quote=F)
        }
    ...
    foldersToCheck<-grep("^vcf_folder",list.files("./vcfs/"),value=T)
    runningJobCount<-0
    for(folderToCheck in foldersToCheck){
      job_status_file<-paste("./vcfs/",folderToCheck,"/job_status.txt",sep="")
      if(file.exists(job_status_file)){
        job_status<-read.table(job_status_file,stringsAsFactors=FALSE,header=FALSE,sep="\t")[1,1]
        if(job_status=="Job is running"){runningJobCount<-runningJobCount+1}
      }
...
run_imputation<-function(
  ...
  shapeit="./programs/shapeit.v2.904.3.10.0-693.11.6.el7.x86_64/bin/shapeit"
  plink="./programs/plink"
  impute2="./programs/impute_v2.3.2_x86_64_static/impute2"
  sample_ref="./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3.sample"
  ...
  start_wd<-getwd()
  runDir <- paste0("./imputations/imputation_folder_",uniqueID)
  ...
  for(chr in c("X",as.character(1:22))){
    ...
    cmd3<-paste(shapeit," -check --input-ped step_2_chr",chr,".ped step_2_chr",chr,".map -M ./programs/ALL_1000G_phase1integrated_v3_impute/genetic_map_chr",chr,"_combined_b37.txt --input-ref ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.hap.gz ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz ",sample_ref," --output-log step_2_chr",chr,"_shapeit_log",sep="")
    ...
    cmd4<-paste(shapeit," --input-ped step_3_chr",chr,".ped step_2_chr",chr,".map -M ./programs/ALL_1000G_phase1integrated_v3_impute/genetic_map_chr",chr,"_combined_b37.txt --input-ref ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.hap.gz ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz ",sample_ref," --output-log step_4_chr",chr,"_shapeit_log --exclude-snp step_3_chr",chr,"_exclusions -O step_4_chr",chr,sep="")
    ...
    cmd6<-paste("zcat ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz | tail -n 1 | cut --delimiter=\\  -f 2",sep="")
    ...
    for(i in 1:length(starts)){
      ...
      if(chunk_lines_length == 0){ #if it's zero we should just skip this
        ...
        cmd7<-paste(impute2," -m ./programs/ALL_1000G_phase1integrated_v3_impute/genetic_map_chr",chr,"_combined_b37.txt -h ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.hap.gz -l ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz -known_haps_g step_5_chr",chr,".haps -int ",start," ",end," -Ne 20000 -o step_7_chr",chr,"_",i,sep="")
...
run_bulk_imputation<-function(
  ...
  for(chr in chromosomes){
    ...
    #Then check for strand flips etc. 
    cmd9<-paste(shapeit," -check --input-ped step_2_chr",chr,".ped step_2_chr",chr,".map -M ./programs/ALL_1000G_phase1integrated_v3_impute/genetic_map_chr",chr,"_combined_b37.txt --input-ref ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.hap.gz ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz ",sample_ref," --output-log step_2_chr",chr,"_shapeit_log",sep="")
    ...
    cmd10<-paste(shapeit," --force --input-ped step_3_chr",chr,".ped step_2_chr",chr,".map -M ./programs/ALL_1000G_phase1integrated_v3_impute/genetic_map_chr",chr,"_combined_b37.txt --input-ref ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.hap.gz ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz ",sample_ref," --output-log step_4_chr",chr,"_shapeit_log --exclude-snp step_3_chr",chr,"_exclusions -O step_4_chr",chr,sep="")
    ...
    #detect max length of each chromosome
    cmd13<-paste("zcat ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz | tail -n 1 | cut --delimiter=\\  -f 2",sep="")
    ...
    for(i in 1:length(starts)){
      ...
      if(chunk_lines_length == 0){ #if it's zero we should just skip this
        ...
        cmd14<-paste(impute2," -m ./programs/ALL_1000G_phase1integrated_v3_impute/genetic_map_chr",chr,"_combined_b37.txt -h ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.hap.gz -l ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz -known_haps_g step_5_chr",chr,".haps -int ",start," ",end," -Ne 20000 -o step_7_chr",chr,"_",i,sep="")
      ...
      if(re_run_chunk){
        ...
        for(j in 1:divisions){
          ...
          cmd15<-paste(impute2," -m ./programs/ALL_1000G_phase1integrated_v3_impute/genetic_map_chr",chr,"_combined_b37.txt -h ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.hap.gz -l ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz -known_haps_g step_5_chr",chr,".haps -int ",start_2," ",end_2," -Ne 20000 -o step_7_chr",chr,"_",i,"-",j,sep="")
...
convert_vcfs_to_simple_format<-function(
  ...
  vcf_folder <- paste0("./vcfs/vcf_folder_",uniqueID,"/")
  bed_path<-"./srv/impute-me/imputeme/2021-01-03_common_snps.txt.gz"
  ...
  out_folder<-paste0("./data/",uniqueID)
...
summarize_imputation<-function(
  ...
  #define programs
  gtools="./programs/gtool"
  
  # plink="./programs/plink-1.07-x86_64/plink" #note, as of 2015-08-31 this must be plink 1.07, otherwise we get a bug
  plink="./programs/plink" #note, as of 2020-01-06 - this seems to have been resolved now and we can use the regular plink (1.9)
  ...
  if(exists("is_vcf_file") && is_vcf_file){
    ...
    crontabs<-sub(" .+$","",sub("^.+Rscript ./srv/impute-me/imputeme/","",crontabs))
...
transfer_cleanup_and_mailout<-function(
  ...
  if(is_vcf_file){
    summary_folder<-paste0("./vcfs/vcf_folder_",uniqueID)
  }else{
    summary_folder<-paste0("./imputations/imputation_folder_",uniqueID)  
  }
  ...
  if(serverRole== "Node"){
    cmd5 <- paste("scp -r ./data/",uniqueID," ubuntu@",hubAddress,":./data",sep="")
    out5<-system(cmd5)
  ...
  if(serverRole== "Node" & !is_vcf_file){
    cmd6 <- paste("ssh ubuntu@",hubAddress," 'ln -s ./data/",uniqueID,"/",uniqueID,".simple_format.zip ./srv/impute-me/www/",uniqueID,".simple_format.zip'",sep="")
    out6<-system(cmd6)
    
    cmd7 <- paste("ssh ubuntu@",hubAddress," 'ln -s ./data/",uniqueID,"/",uniqueID,".gen.zip ./srv/impute-me/www/",uniqueID,".gen.zip'",sep="")
    out7<-system(cmd7)
    
    cmd8 <- paste("ssh ubuntu@",hubAddress," 'ln -s ./data/",uniqueID,"/",uniqueID,"_data.json ./srv/impute-me/www/",uniqueID,"_data.json'",sep="")
    out8<-system(cmd8)
    ...
    file.symlink(
      ...
      to=paste("./srv/impute-me/www/",uniqueID,"_data.json",sep="")
    )
    ...
    file.symlink(
      ...
      to=paste("./srv/impute-me/www/",uniqueID,".simple_format.zip",sep="")
    )
    file.symlink(
      from=paste("./data/",uniqueID,"/",uniqueID,".gen.zip",sep=""),
      to=paste("./srv/impute-me/www/",uniqueID,".gen.zip",sep="")
    )
    file.symlink(
      from=paste("./data/",uniqueID,"/",uniqueID,"_data.json",sep=""),
      to=paste("./srv/impute-me/www/",uniqueID,"_data.json",sep="")
  ...
  if(serverRole== "Node"){
    cmd9 <- paste("ssh ubuntu@",hubAddress," 'rm -r ./imputations/imputation_folder_",uniqueID,"'",sep="")
    out9<-system(cmd9)
...
get_genotypes<-function(
  ...
  if(class(uniqueID)!="character")stop(paste("uniqueID must be character, not",class(uniqueID)))
  if(length(uniqueID)!=1)stop(paste("uniqueID must be lengh 1, not",length(uniqueID)))
  idFolder<-paste("./data",uniqueID,sep="/")
  ...
  idTempFolder<-paste("./data",uniqueID,"temp",sep="/")
...
crawl_for_snps_to_analyze<-function(
  ...
  for(module in list.files("./srv/impute-me",full.names=T)){
  ...
  for(uniqueID in uniqueIDs){
    ...
    if(class(genotypes)=="try-error"){
      if(file.exists(paste("./data/",uniqueID,"/temp",sep=""))){
        next
      }else{
        if(verbose>0){
  ...
  e<-try(load("./srv/impute-me/nonsenser/2015-12-16_all_coding_SNPs.rdata"))
  ...
  load("./srv/impute-me/AllDiseases/2020-04-02_all_gwas_snps.rdata")
  e1<-gwas_snps
  load("./srv/impute-me/ukbiobank/2017-09-28_all_ukbiobank_snps.rdata")
  e2<-gwas_snps
  ...
  e<-try(load("./srv/impute-me/ethnicity/2017-04-03_ethnicity_snps.rdata"))
  if(class(e)!="try-error"){
    for(uniqueID in uniqueIDs){
...
make_overview_of_samples<-function(
  ...
  if(type == "bash"){
    cmd1 <- paste(c("echo -n > ./misc_files/temporary_file_for_overview.txt;",
                    "for filename in ./data/id_*;",
                    "do",
                    "file=$filename/pData.txt;",
                    "cat $file >> ./misc_files/temporary_file_for_overview.txt;",
                    "done"),collapse=" ")
    system(cmd1)
    d<-readLines("./misc_files/temporary_file_for_overview.txt")
    unlink("./misc_files/temporary_file_for_overview.txt")
    d1<-strsplit(d,"\t")
...
remove_snps_from_cache<-function(
  ...
  for(uniqueID in uniqueIDs){
    cacheFile<-paste("./data/",uniqueID,"/",uniqueID,".cached.gz",sep="")
...
generate_report<-function(
  ...
  filepath <- paste0("./srv/impute-me/www/",filename)
  ...
  for(uniqueID in uniqueIDs){
    ...
    #getting pData file
    pData_file<-paste("./data",uniqueID,"pData.txt",sep="/")
  ...
  # for(w1 in list.files("./imputations",full.names=T)){
...
run_export_script<-function(
  ...
  if(is.null(modules)){
    modules<-list.files("./srv/impute-me/")
  }else{
    if(class(modules)!="character")stop("modules must be of class character")
    if(!all(file.exists(paste("./srv/impute-me/",modules,sep=""))))stop("Not all modules given were found")
  }
  ...
  for(uniqueID in uniqueIDs){
    ...
    cachedFile<-paste("./data/",uniqueID,"/",uniqueID,".cached.gz",sep="")
    ...
    if(!"ethnicity"%in%colnames(pData)){
      source(paste(paste0("./srv/impute-me/","ethnicity"  ,"/export_script.R")))
      ...
      pDataFile <- paste0("./data/",uniqueID,"/pData.txt")
      pData<-try(read.table(pDataFile,header=T,stringsAsFactors=F,sep="\t"),silent=T)
      if(class(pData)!="try-error"){
    ...
    for(module in modules){
      if(!file.info(paste0("./srv/impute-me/",module))["isdir"])next
      if("export_script.R" %in% list.files(paste0("./srv/impute-me/",module))){
        
        print(paste0(Sys.time(),": Running module ",module," for ",uniqueID))
        if(exists("export_function"))suppressWarnings(rm("export_function"))
        source(paste(paste0("./srv/impute-me/",module,"/export_script.R")))
    ...
    filename <- paste0("./data/",uniqueID,"/",paste(uniqueID,"data.json",sep="_"))
...
reset_runs_from_node<-function(
  ...
  if(check_is_running){
    for(uniqueID in uniqueIDs){
      cmd1 <- paste0("ssh ubuntu@",hubAddress," 'cat ./imputations/imputation_folder_",uniqueID,"/job_status.txt'")
  ...
  for(uniqueID in uniqueIDs){
    cmd2 <- paste0("ssh ubuntu@",hubAddress," 'echo Job is ready > ./imputations/imputation_folder_",uniqueID,"/job_status.txt'")
    system(cmd2)
  }
...
summarize_imputemany_json<-function(
  ...
  for(uniqueID in uniqueIDs){
    if(!file.exists(paste0("./data/",uniqueID)))missing_files<-c(missing_files,uniqueID)
  }
  if(length(missing_files)>0)  stop(paste("These",length(missing_files),"uniqueIDs were missing:",paste(missing_files,collapse=", ")))
  missing_json <- vector()
  for(uniqueID in uniqueIDs){
    json_path <- paste0("./data/",uniqueID,"/",uniqueID,"_data.json")
  ...
  trait_path1 <- "./srv/impute-me/AllDiseases/2020-04-02_trait_overview.xlsx"
  library(openxlsx)
  ...
  trait_path2 <- "./srv/impute-me/ukbiobank/2017-09-28_trait_overoverview.rdata"
  load(trait_path2)
  ...
  file_out_long <- paste0("./srv/impute-me/",file_out)
...
check_for_rare_nonbiallic_snps<-function(
  ...
  if(length(uniqueID)!=1)stop(paste("uniqueID must be length 1, not",length(uniqueID)))
  basefolder <- paste0("./imputations/imputation_folder_",uniqueID)
  ...
  for(chr in c("X",as.character(1:22))){
    ...
    #running the shapeit command (with two people, the right one and a placeholder heterozygote
    cmd4<-paste(shapeit," -check --input-ped step_3_chr",chr,".ped step_2_chr",chr,".map -M ./programs/ALL_1000G_phase1integrated_v3_impute/genetic_map_chr",chr,"_combined_b37.txt --input-ref ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.hap.gz ./programs/ALL_1000G_phase1integrated_v3_impute/ALL_1000G_phase1integrated_v3_chr",chr,"_impute.legend.gz ",sample_ref," --output-log step_4_chr",chr,"_shapeit_log --exclude-snp step_3_chr",chr,"_exclusions",sep="")
